...
#define USB_PAYLOAD_SIZE 96
#define PACKET_HEADER_SIZE 4
#define USB_TX_BUFFER_SIZE (USB_PAYLOAD_SIZE + PACKET_HEADER_SIZE)
#define PACKET_START_MARKER1 0xAA
#define PACKET_START_MARKER2 0x55

#define AUDIO_BUFFER_SIZE 1024                // words (uint16_t)
uint16_t audio_buffer[AUDIO_BUFFER_SIZE * 2]; // Ping-pong buffer
...
// ==== FIFO ====
void FifoWrite(uint8_t data)
{
  uint16_t next = (fifo_w_ptr + 1) % FIFO_SIZE;
  if (next == fifo_r_ptr)
  {
    fifo_r_ptr = (fifo_r_ptr + 1) % FIFO_SIZE; // drop oldest
  }
  fifobuf[fifo_w_ptr] = data;
  fifo_w_ptr = next;
}

uint8_t FifoRead(void)
{
  if (fifo_r_ptr != fifo_w_ptr)
  {
    uint8_t val = fifobuf[fifo_r_ptr];
    fifo_r_ptr = (fifo_r_ptr + 1) % FIFO_SIZE;
    return val;
  }
  return 0;
...
void FifoWriteFromBuffer(uint16_t *buffer, uint16_t start, uint16_t end)
{
  for (uint16_t i = start; i < end; i += 4)
  {
    ...
    uint8_t byte0 = sample_24bit & 0xFF;
    uint8_t byte1 = (sample_24bit >> 8) & 0xFF;
    uint8_t byte2 = (sample_24bit >> 16) & 0xFF;
...
// ==== Send USB Packet if ready ====
void SendFifoPacketIfReady(void)
{
  if (usbTxReady && FifoAvailable() >= USB_PAYLOAD_SIZE)
  {
    ...
    for (uint16_t i = 0; i < USB_PAYLOAD_SIZE; i++)
    {
      usb_tx_buffer[i + PACKET_HEADER_SIZE] = FifoRead();
    }

    usbTxReady = 0;
    CDC_Transmit_FS(usb_tx_buffer, USB_TX_BUFFER_SIZE);
  }
}

// ==== Simulate I2S DMA Callbacks ====
void HAL_I2S_RxHalfCpltCallback1(void)
{
  current_buffer = 1;
  FifoWriteFromBuffer(audio_buffer, 0, AUDIO_BUFFER_SIZE);
}

void HAL_I2S_RxCpltCallback1(void)
{
  current_buffer = 2;
  FifoWriteFromBuffer(audio_buffer, AUDIO_BUFFER_SIZE, AUDIO_BUFFER_SIZE * 2);
}
...
//  {
//    audio_buffer[i] = 0x1200 + (i & 0xFF);     // LH
//    audio_buffer[i + 1] = 0x0033 + (i & 0xFF); // LL
//    audio_buffer[i + 2] = 0x5000;              // RH (bỏ)
//    audio_buffer[i + 3] = 0x0011;              // RL (bỏ)
//  }
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    // Mô phỏng I2S DMA
    HAL_I2S_RxHalfCpltCallback1();
    HAL_I2S_RxCpltCallback1();

    // Gửi nếu đủ 144 byte
    while (FifoAvailable() >= USB_PAYLOAD_SIZE)
    {
      SendFifoPacketIfReady();
    }
    CheckUSBStatus();
  }
  /* USER CODE END 3 */
