...
volatile uint32_t fifo_w_ptr = 0;
volatile uint32_t fifo_r_ptr = 0;
uint8_t fifo_read_enabled = 0;
#define USB_PAYLOAD_SIZE 300
...
int main(void)
{
  ...
  HAL_I2S_Receive_DMA(&hi2s3, (uint16_t *)audio_buffer, AUDIO_BUFFER_SIZE * AUDIO_CHANNELS * 2);
  /* USER CODE END 2 */
